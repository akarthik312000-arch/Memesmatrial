...
function aiContextBlock(): string {
  if (!compacted && usage.totalTokens >= TOKEN_COMPACT_THRESHOLD && history.length > 5) {
    ...
    compactionCount++;
    console.log(
      `[MM] ${TOKEN_COMPACT_THRESHOLD / 1000}k token budget reached - ` +
...
async function renderVideo(
  ...
  await runProc(FFMPEG, args, 300000);
  return { bgSource };
}
...
export async function validateOutput(p: string, expectedSec: number): Promise<{ valid: boolean; error?: string; meta: Record<string, unknown> }> {
...
function youtubeData(form: VideoCreationForm, c: Concept) {
  ...
  return {
    ...
    pinnedComment: "Which one are you? Comment below",
...
export type PipelineOptions = {
  /** editor-provided scene script; skips concept generation */
  scenesOverride?: Array<{ text: string; voice: string; visual?: string }>;
  skipConceptAI?: boolean;
  onProgress?: (stage: string) => void;
};
...
export async function runVideoPipeline(
  ...
  try {
    let concept: Concept;
    let scenes: Array<Scene & { _sub?: string; _visual?: string }>;

    if (opts.scenesOverride?.length) {
      // editor-driven re-render: keep the user's script exactly
      concept = buildConcept(form);
      scenes = opts.scenesOverride.slice(0, 20).map((s, i) => ({
        id: `scene-${i}`,
        duration: SCENE_SEC,
        visualPrompt: s.text.slice(0, 60),
        voiceOver: s.voice.slice(0, 200),
        subtitles: s.voice.slice(0, 200),
        sfx: [] as string[],
        musicTransition: i > 0,
        _sub: i % 4 === 4 ? "follow for more" : "memesmaterial",
        _visual: s.visual ?? "",
      }));
    } else {
      ...
      scenes = aiScenes && aiScenes.length >= 3 ? aiScenes : buildScenes(concept);
    }
    ...
    const { bgSource } = await renderVideo(tmpDir, scenes, form, concept.character, tmpFinal, totalSec, opts.onProgress);
...
/** SRT/VTT subtitle cues from scene narration lines */
export function buildSubtitles(
  scenes: Array<{ voiceOver?: string; subtitles?: string; duration?: number }>,
  totalSec: number
): { srt: string; vtt: string } {
  function fmt(sec: number, comma: boolean): string {
  ...
  const per = scenes.length ? totalSec / scenes.length : totalSec;
  let srt = "";
  let vtt = "WEBVTT\n\n";
  scenes.forEach((sc, i) => {
    const start = i * per;
    const end = Math.min((i + 1) * per, totalSec);
    const text = (sc.subtitles || sc.voiceOver || "").trim();
    if (!text) return;
    srt += `${i + 1}\n${fmt(start, true)} --> ${fmt(end, true)}\n${text}\n\n`;
    vtt += `${fmt(start, false)} --> ${fmt(end, false)}\n${text}\n\n`;
  });
  return { srt, vtt };
}
